...
class MoviesAugDataset(Dataset):
    def __init__(self):
        super().__init__()

        self.graph = torch.load(self.processed_file_names[0])
        self.text = self.graph.raw_texts
        self.synthetic_summary = self.graph.abs
        ...
        candidates_str = ', '.join(candidates["Movies"])
    ...
    def processed_file_names(self) -> str:
        return ['../../datasets/Movies/Movies_Aug_struct_graph_data.pt']

    def get_idx_split(self):    
        json_path = "../../datasets/Movies/Movies_split.json"
