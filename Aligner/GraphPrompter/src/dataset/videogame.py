...
candidates = {
    ...
    "CD": ['Pop', "Today's Deals in Music", 'Rock', 'Indie & Alternative', 'Classic Rock', 'Country', 'International Music', 'Jazz', 'Metal', 'R&B', 'Classical', 'Rap & Hip-Hop', 'Christian & Gospel', 'Blues', 'Dance & Electronic'],
    "VideoGame": ['Legacy Systems', 'PC', 'Nintendo Switch', 'PlayStation 4', 'Xbox One', 'PlayStation 5']
} 
...
class VideoGameDataset(Dataset):
    def __init__(self):
        ...
        candidates_str = ', '.join(candidates["VideoGame"])
    ...
    def processed_file_names(self) -> str:
        return ['../..datasets/VideoGames/VideoGame_graph_data.pt']

    def get_idx_split(self):    
        json_path = "../..datasets/VideoGames/split.pt"
        loaded_data_dict = torch.load(json_path)

        train_ids = [int(i) for i in loaded_data_dict['train_idx']]
        val_ids = [int(i) for i in loaded_data_dict['val_idx']]
        test_ids = [int(i) for i in loaded_data_dict['test_idx']]
