...
def eval_mgllm_nc(dsname, res_path):
    data=torch.load(f"../dataset/{dsname}/{dsname}Graph_cliptext.pt")
    ys=data.y.numpy().tolist()
    category = data.category_label_mapping
    # print("len(categories)", len(category))
    labels=[category[i] for i in ys]
    short_labels = [l.split('_')[0] for l in labels]
    ...
    with open(res_path, 'r') as f:
        for line in f:
            ...
            y=ys[res["question_id"]]
            label=labels[nid]
            # print("question_id:", res["question_id"], "y:", y, "label:", label, "ans:", ans)
            short_label=short_labels[nid]
            if short_label.strip().lower() == ans.strip().lower(): # and sum([l.strip().lower() in ans.strip().lower() for l in short_labels])==1:
                correct+=1
    acc=correct/all_sample
    print(f"Test samples: {all_sample}\nacc: {acc:.4f}\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--res_file", type=str, default="nofile.txt")
    ...
    if args.dataset == "mgllm":
     for dataset in ["Movies", "Toys", "Grocery", "Arts", "CD"]: #"Reddit", 
        print(f"**********Dataset: {dataset}**********")
        for feat_type in ["augnonstruc2", "augnonstruc3"]:#["cliptext", "clipimage", "cliptextimage", "clipstruc", "clipnonstruc"]:#, "imagebind", "imagebindtext", "imagebindimage"]:
            print(f"Feature type: {feat_type}")
            func=func_dict["mgllm"][args.task]
            res_path = args.res_path + f"/{dataset}/{dataset}_{feat_type}_label.jsonl"
            # if exist res_path
            if os.path.exists(res_path):
                func(dataset, res_path)
            else:
                print(f"{res_path} not exists\n")
    else:
        if args.dataset in ["Movies", "Toys", "Grocery", "Reddit", "Arts", "CD"]:
            print(f"**********Dataset: {args.dataset}**********")
            if os.path.exists(args.res_file):
                print("args.res_file:", args.res_file)
                func=func_dict["mgllm"][args.task]
                func(args.dataset, args.res_file)
            else:
                print(f"{args.res_file} not exists\n")
        else:
