...
def load_pretrain_embedding_graph(data_dir, pretrained_embedding_type):
    if pretrained_embedding_type == "simteg":
        ...
        if os.path.exists(os.path.join(data_dir, f"{pretrained_embedding_type}_x.pt")):
            ...
            print(f"load_pretrain_embedding_graph: {pretrained_embedding_type} not exists")
            raise ValueError
...
def eval_model(args):
    ...
    elif args.dataset in["Movies", "Toys", "Grocery", "Reddit", "Arts", "CD"]:
    ...
    if args.task in  ["nc", "nd", "nda", "nctext"]:
        ...
        if args.dataset in ["Movies", "Toys", "Grocery", "Reddit", "Arts", "CD"]:
            data_path = os.path.join(data_dir, f"{args.dataset}Graph_cliptext.pt")
