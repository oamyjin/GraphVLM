...
class LazySupervisedGraphDataset(Dataset):
    ...
    def __init__(self,
        ...
        print("LazySupervisedGraphDataset use_dataset", self.use_dataset)
        ...
        for d, dataset in enumerate(self.use_dataset):
            ...
            elif "Movies" in dataset or "Toys" in dataset or "Grocery" in dataset or "Reddit" in dataset or "Arts" in dataset or "CD" in dataset:
                data_path = f"../dataset/{dataset}/{dataset}Graph_cliptext.pt"
            else:
            ...
            for task in self.use_task:
                ...
                if task == "nc":
                    ...
                    print("os.path.exists(data_path):", os.path.exists(data_path))
    ...
    def load_pretrain_embedding_graph(self, data_dir, pretrained_embedding_type):
        if pretrained_embedding_type == "simteg":
            ...
            # check if exist: os.path.join(data_dir, f"{pretrained_embedding_type}_x.pt")
            if os.path.exists(os.path.join(data_dir, f"{pretrained_embedding_type}_x.pt")):
                pretrained_emb = torch.load(os.path.join(data_dir, f"{pretrained_embedding_type}_x.pt"))
            elif os.path.exists(os.path.join(data_dir, f"{pretrained_embedding_type}_feat.pt")):
                pretrained_emb = torch.load(os.path.join(data_dir, f"{pretrained_embedding_type}_feat.pt"))
            else:
                print(f"{pretrained_embedding_type} not exists!")
                f_path = os.path.join(data_dir, f"{pretrained_embedding_type}_feat.pt")
                print(f"no {f_path}")
                raise ValueError
        
        if pretrained_emb.device != torch.device('cpu'):
            pretrained_emb = pretrained_emb.to('cpu')
        print(f"{pretrained_embedding_type} pretrained_emb.shape:{pretrained_emb.shape}")
...
def _train():
    ...
    elif data_args.pretrained_embedding_type in ["simteg_e5", "simteg_roberta", "roberta", "imagebindtext", "imagebindimage"]:
        model_args.mm_hidden_size = 1024
    elif data_args.pretrained_embedding_type in ["simteg"]:
        model_args.mm_hidden_size = 1024*2+384
    elif data_args.pretrained_embedding_type in ["cliptext", "clipimage", "augnonstruc", "augstruc"]:
        model_args.mm_hidden_size = 768
    elif data_args.pretrained_embedding_type in ["cliptextimage", "clipstruc", "clipnonstruc", "augnonstruc2"]:
        model_args.mm_hidden_size = 768*2
    elif data_args.pretrained_embedding_type in ["augnonstruc3"]:
        model_args.mm_hidden_size = 768*3
    elif data_args.pretrained_embedding_type in ["imagebind"]:
        model_args.mm_hidden_size = 1024*2
        
    else:
        print(f"{data_args.pretrained_embedding_type} not exists!")
        raise ValueError
    if data_args.template == "ND":
    ...
    #     print("qwen in ", model_args.model_name_or_path)
    #     model = LlagaQwenForCausalLM.from_pretrained(
    #         model_args.model_name_or_path,
    #         cache_dir=training_args.cache_dir,
